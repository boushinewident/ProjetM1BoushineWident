...
to mettreAJourEstIsole
  ask turtles [
    ...
    let nbAgentsDeLaCoalition count (turtles-on neighbors) with [numCoalition = numCoalitionPrincipal]
...
to MouvementCoalitions
  ask turtles[
    ...
    ifelse hasLeader[
      ...
      ifelse count (turtles-on neighbors) with [numCoalition = numCoalitionPrincipal] = 0 ;; est isolé
      [
        DevenirIndependant
      ]
      [
...
to-report getNumeroMeilleurVoisin
  let listePayoff [numAgent] of (turtles-on neighbors) with-max[payOff]
  report item 0 listePayoff
end 


to-report worstPayOff
  let listePayoff [payoff] of (turtles-on neighbors) with-min[payOff]
